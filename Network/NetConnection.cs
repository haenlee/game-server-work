...
namespace Network
{
    public abstract class NetConnection
    {
        ...
        public NetConnection(Socket socket, NetSession session, EventHandler<NetSession> onSocketDisconnected, ReceiveDataCallback receiveCallback)
        {
            Session = session;
        ...
        {
            _receiveCallback?.Invoke(data, offset, count);
            ...
            while (true)
            {
                ...
                // 패킷 메시지 파싱
                OnParseProto(stream.GetBuffer(), readOffset + Config.HEADER_SIZE, dataSize);
        ...
        public abstract void OnParseProto(byte[] buffer, int readOffset, int dataSize);
        #endregion
